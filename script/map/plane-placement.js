...
export async function showPreciseFlightInfo(pilot) {
    ...
    if (pilot.flight_plan === null) {
        ...
        $('.dep-time').text(
            `${pilot.flight_plan.deptime.substring(0, 2)}:${pilot.flight_plan.deptime.substring(2)}`,
        );
        ...
        $('.arv-time').text(
            `${arrivalH.toString().padStart(2, '0')}:${arrivalM.toString().padStart(2, '0')}`,
        );
    }
    ...
    if (pilot.flight_plan) {
        ...
        registrationText = pilot.flight_plan.remarks
            .toLowerCase()
    ...
    function setCountryDetails(country, size = '19px') {
        ...
        $('.aircraft-details-aircraft-registration-country')
            .text(country.name)
            .css({ left: '372px', 'font-size': size });
    ...
    function handleNoMatchingCountry() {
        console.log(
            'No matching country found for the given registration prefix with two characters.',
        );
        $('.aircraft-details-aircraft-registration-country-flag')
            .attr('src', '')
            .css('width', '0');
        $('.aircraft-details-aircraft-registration-country')
            .text('N/A')
            .css({ left: '362px', 'font-size': 'var(--font-size-1)' });
    }
    ...
    try {
        ...
        const matchingCountry = countriesData.find(
            (country) => country.reg === registrationPrefix,
        );
        ...
        if (pilot.flight_plan) {
            if (matchingCountry) {
                ...
                // Check for one character registration prefix
                const oneCharRegistrationPrefix = registrationText.substring(
                    0,
                    1,
                );
                const oneCharMatchingCountry = countriesData.find(
                    (country) => country.reg === oneCharRegistrationPrefix,
                );
            ...
            console.error(
                "No flight plan, cannot get the aircraft's registration.",
            );
            $('.aircraft-details-aircraft-registration-country').text('N/A');
            $('.aircraft-details-aircraft-registration-country-flag').css(
                'width',
                '0',
            );
        }
        ...
        console.error(
            'Error fetching countries data, most likely the aircraft does not have its registration filled in remarks.',
        );
        $('.aircraft-details-aircraft-registration-country')
            .text('N/A')
            .css({ left: '362px', 'font-size': 'var(--font-size-1)' });
    ...
    const knownBadRegistrations = ['PMDG738', 'PMDG739', 'B747'];
